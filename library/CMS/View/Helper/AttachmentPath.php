 ...
 */
class CMS_View_Helper_AttachmentPath extends Zend_View_Helper_HtmlElement {
     ...
     * @return string
     */
    public function attachmentPath($source) {
        $source = strtolower($source);
        $config = Zend_Registry::get('config');
        $salt = $config->site->salt;

        $hash = attachmentHash($source);
        $source=urlencode($source);

        $array = $config->resources->router->routes->toArray();
        $route = $array['core-attachment']['route'];
        $find = array(':source', ':hash');
        $replace = array($source, $hash);
